...
class TripletAttentionModule(LightningModule):
    def __init__(
        ...
        # self.tool_criterion = focal_binary_cross_entropy()
        # self.verb_criterion = focal_binary_cross_entropy()
        # self.target_criterion = focal_binary_cross_entropy()
        # self.triplet_criterion = focal_binary_cross_entropy()

        '''torch.hub.load(
                                    'adeelh/pytorch-multi-class-focal-loss',
                                    model='FocalLoss',
                                    alpha=torch.tensor([focal_loss_hyper.triplet.alpha,1-focal_loss_hyper.triplet.alpha]),
                                    gamma=focal_loss_hyper.triplet.gamma,
                                    reduction='mean',
                                    force_reload=False
        )'''
    ...
    def step(self, batch: Any):
        ...
        *phase*: the phase [phase 0, ..., phase 6]
        ...
        tool_loss = focal_binary_cross_entropy(tool_logit, batch["tool"], self.class_num["tool"], self.hparams.fbce_hyper.tool.gamma)
        target_loss = focal_binary_cross_entropy(target_logit, batch["target"], self.class_num["target"], self.hparams.fbce_hyper.target.gamma)
        verb_loss = focal_binary_cross_entropy(verb_logit, batch["verb"], self.class_num["verb"], self.hparams.fbce_hyper.verb.gamma)
        triplet_loss = focal_binary_cross_entropy(triplet_logit, batch["triplet"], self.class_num["triplet"], self.hparams.fbce_hyper.triplet.gamma)
        return (
            (
