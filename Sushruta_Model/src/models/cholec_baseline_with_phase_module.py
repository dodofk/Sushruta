...
class TripletBaselineModule(LightningModule):
    def __init__(
        ...
        self.train_phase_map = Precision(
            num_classes=self.class_num["phase"],
            average="macro",
        )
    ...
    def forward(self, x):
        ...
        # adding phase to train model 
        phase_logit = self.phase_head(tool_info)
        
        return tool_logit, target_logit, verb_logit, triplet_logit, phase_logit
    ...
    def step(self, batch: Any):
        ...
        *phase*: the phase [phase 0, ..., phase 6]
        ...
        """
        tool_logit, target_logit, verb_logit, triplet_logit, phase_logit = self.forward(
            batch["image"]
        )
        ...
        phase_loss = self.criterion(phase_logit, batch["phase"])
        return (
            ...
            + self.hparams.loss_weight.phase_weight * phase_loss,
            tool_logit,
            target_logit,
            verb_logit,
            triplet_logit,
    ...
    def training_step(self, batch: Any, batch_idx: int):
        loss, tool_logit, target_logit, verb_logit, triplet_logit, phase_logit = self.step(batch)
