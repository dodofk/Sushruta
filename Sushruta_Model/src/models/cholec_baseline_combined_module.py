...
class TripletBaselineModule(LightningModule):
    def __init__(
        ...
        self.train_target_combined_map = Precision(
            num_classes=len(self.target_to_combined),
            average="macro",
        )
        ...
        self.target_combined = nn.Sequential(
            nn.Linear(self.feature_extractor.num_features, len(self.target_to_combined)),
        )
    ...
    def target_combine_transform(self, target: torch.Tensor):
        new_target = torch.zeros(target.size(0), len(self.target_to_combined)).to(target.device)
        for i in range(0, target.size(0)):
            for j in range(0, len(self.target_to_combined)):
